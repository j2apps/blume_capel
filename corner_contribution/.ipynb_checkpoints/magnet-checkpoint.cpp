...
array<double, 3> run_single_run(const string& input_dirname, int L) {
    ...
    const double m0_avg = m0/num_samples;
    const double m1_avg = m1/num_samples;
    const double m2_avg = m2/num_samples;
    array<double, 3> result = {m0_avg, m1_avg, m2_avg - m1_avg*m1_avg};
    return result;
}
...
void run_statistics(const string& input_root, const string& output_root, const int n_batches, const int n_runs) {
    string output = "batch,L,m0,se_m0,m,se_m,X,se_X\n";
    for (int l: {16, 24, 32, 48, 64, 96}) {
        ...
        vector<double> m0(n_runs);
        vector<double> m(n_runs);
        vector<double> x(n_runs);
        #pragma omp parallel for num_threads(NUM_THREADS)
        for (int run = 0; run < n_runs; run++) {
            array<double, 3> result = run_single_run(input_dirnames[run], l);
            m0[run] = result[0] / (l*l);
            m[run] = result[1] / (l*l);
            x[run] = result[2] / (l*l);
        }
