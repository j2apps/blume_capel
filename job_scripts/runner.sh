...
for i in {0..6};
do
	sbatch --cpus-per-task=${cores[i]} --partition=${partition[i]} --time="${time[i]}:00:00" --mem=${mem[i]} --job-name="${5}-${sizes[i]}" --array="25-$((nruns[i]-1))" job_scripts/run.sh ${sizes[i]} $1 ${nsamples[i]} $2 $3 $4 $5
done
